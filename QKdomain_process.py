...
for gene in gene_position_domain.keys():
    ...
    if len(options.domain) > 0:
        ...
        for index in range(0, len(gene_structure) - num_domains + 1):
            if options.domain == '-'.join(gene_structure[index:(index+num_domains)]):
                ...
                if len(args) > 4:
                    if occurrence == 1:
                        # if the number of domains equals one, use original name
                        gene_name = gene
                    elif domain_occurrence < 26:
                        # if the number of domains exceeds 26, use one character suffix
                        gene_name = gene + chr(domain_occurrence + 97)
                    else:
                        # if the number of domains exceeds 26, use two character suffix
                        gene_name = gene + chr(domain_occurrence / 97) + chr(domain_occurrence % 97)
                    
                    individual_domain_names[gene].append(gene_name)
                    
                    individual_domain_file.write('>' + gene_name + ' ' + str(local_start) + ',' + str(local_stop) + ' ' + '-'.join(gene_structure) + '\n')
    ...
    if len(options.iTOL) > 0:
        if len(gene_structure) > 0:
            if len(options.domain) > 0:
                for individual_domain in individual_domain_names[gene]:
                    iTOL.write(individual_domain + ',' + str(len(ID_sequence[gene])))
